...
def run_scp(T=11.):
    ...
    pathToModel = "/home/jalora/Desktop/trunk_origin/000/SSMmodel_delay-embedding_ROMOrder=3_localV"
    # Simulation settings
...
def run_gusto_solver_call(taskParams, model):
    ...
    # Reshape desired trajectory based on SSM constraints
    if taskParams['z'].shape[1] < model.output_dim:
        taskParams['z'] = np.hstack((taskParams['z'], np.zeros((taskParams['z'].shape[0], model.output_dim - taskParams['z'].shape[1]))))
    

    obstacleLoc = [np.asarray(vector) for vector in taskParams['obstacleLoc']]
    obstacleDiameter = [diameter for diameter in taskParams['obstacleDiameter']]

    taskParams['X'] = createObstacleConstraint(model.output_dim, model.y_ref, obstacleDiameter, obstacleLoc)
...
def run_scp_call(modelType, model, dt, T=11.):
    """
     In problem_specification add:
    ...
    # dt = 0.02 # TODO: Modify this
    save_prefix = modelType
    ...
    ######## Specify a measurement of what we observe during simulation ########
    cov_q = 0.001 * np.eye(3)
    cov_v = 30.0 * np.eye(3) # 60.0 * np.eye(3)
    prob.output_model = prob.Robot.get_measurement_model(nodes=[TIP_NODE])
    if model.params['delay_embedding']:
        prob.measurement_model = MeasurementModel(nodes=[TIP_NODE], num_nodes=N_NODES, pos=True, vel=False, S_q=cov_q)
    else:
