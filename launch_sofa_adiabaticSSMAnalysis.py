...
SAVE_DIR = join("/media/jonas/Backup Plus/jonas_soft_robot_data/trunk_adiabatic_analysis", split(MODEL_DIR)[1])
...
def doSimulations():
    ...
    for i in tqdm(range(ADD_MODELS_PER_STEP, len(model_names) + 1, ADD_MODELS_PER_STEP)):
        for j in tqdm(range(SIMS_PER_MODEL)):
            # add the next {add_models_per_step} models to use_models
            use_models = [0] + np.random.choice([int(name) for name in model_names], size=(i - 1), replace=False).tolist() # [int(model_names[j]) for j in list(range(i))]
            # save use_models as .pkl file
            save_dir_i = join(SAVE_DIR, f"{i:03d}")
            if exists(join(save_dir_i, f"ssmr_sim_{j}.pkl")):
                continue
            else:
...
def plotResults():
    ...
    solve_times = {}
    n_models = {}
    for control in ["idw_no-filter", "idw", "qp"]:
        z[control] = []
        solve_times[control] = []
        n_models[control] = []
    ...
    plot.rmse_vs_n_models(n_models, z, solve_times, save_dir=split(SAVE_DIR)[0])
