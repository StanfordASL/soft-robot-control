...
class SSMGuSTO(TemplateModel):
    ...
    def get_continuous_dynamics(self, x, u):
        ...
        if hasattr(self.dyn_sys, "adiabatic") and self.dyn_sys.adiabatic:
            R = self.dyn_sys.interpolator.transform(x[:], "r_coeff")
            B_r = self.dyn_sys.interpolator.transform(x[:], "B_r")
            u_bar = self.dyn_sys.interpolator.transform(x[:], "u_bar")
            A, B, d = self.dyn_sys.get_continuous_jacobians(x, u, R, B_r, u_bar) #  self.dyn_sys.R_current, self.dyn_sys.B_r_current, self.dyn_sys.u_bar_current)
        else:
    ...
    def get_discrete_dynamics(self, x, u, dt):
        ...
        if hasattr(self.dyn_sys, "adiabatic") and self.dyn_sys.adiabatic:
            ...
            B_r = self.dyn_sys.interpolator.transform(x[:], "B_r")
            u_bar = self.dyn_sys.interpolator.transform(x[:], "u_bar")
            return self.dyn_sys.get_jacobians(x, u, dt, R, B_r, u_bar) #  self.dyn_sys.R_current, self.dyn_sys.B_r_current, self.dyn_sys.u_bar_current)
        else:
    ...
    def get_observer_jacobians(self, x, u=None, dt=None):
        ...
        """
        if hasattr(self.dyn_sys, "adiabatic") and self.dyn_sys.adiabatic:
            W = self.dyn_sys.interpolator.transform(x[:], "w_coeff")
            y_bar = np.tile(self.dyn_sys.interpolator.transform(x[:], "q_bar"), 5)
            return self.dyn_sys.get_observer_jacobians(x, W, y_bar) # self.dyn_sys.W_current, self.dyn_sys.y_bar_current)
        else:
