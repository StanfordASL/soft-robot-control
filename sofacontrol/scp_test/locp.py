...
class LOCP:
    ...
    def __init__(self, N, H, Qz, R, Qzf=None, U=None, X=None, Xf=None, dU=None, verbose=False, warm_start=True,
                 x_char=None, **kwargs):
        self.N = N
        # TODO: Use this H if nonlinear_observer = False
        self.H = H
        self.Qz = Qz
        self.R = R
        self.Qzf = Qzf
        ...
        # TODO: Need to add this as a parameter that is propogated
        self.nonlinear_observer = kwargs.pop('nonlinear_observer', False)

        # TODO: Ensure we have a self.H in SSM class such that 2nd dim is dim of RO state
        self.n_x = H.shape[1]
        self.n_z = Qz.shape[0]
        self.n_u = R.shape[0]
        ...
        if self.warm_start:
            ...
            # TODO: Adding observer linearization parameters here. Expect parameters to be None
            # TODO: if dynamics class has nonlinear_observer = False. In this case this class should
            # TODO: use self.H as shown above. This seems to be different from when not warm_starting
    ...
    def update(self, Ad, Bd, dd, x0, xk, delta, omega, z=None, zf=None, u=None, full=True, **kwargs):
        ...
        if self.warm_start:
            ...
            if full:
                ...
                # TODO: Added observer linearizations here. Make sure to propogate Hd and cd as
                # TODO: parameters in kwargs
                ...
                for j in range(self.N + 1):
                    if self.nonlinear_observer:
                        self.Hd[j].value = kwargs.get('Hd')[j]
        ...
        # Otherwise just build a new problem from scratch each time
            ...
            # TODO: Observer params here
            if self.nonlinear_observer:
                self.Hd = kwargs.get('Hd')
    ...
    def set_objective(self):
        ...
        J += cp.quad_form(self.u - self.u_des, psd_wrap(Rfull))
        ...
        # TODO: affine term c_k. This implicitly takes into account mapping from RO state to observed
        # TODO: this is an equivalent optmization problem to imposing the mapping as an equality constraint
        ...
        if self.nonlinear_observer:
            ...
            J += cp.quad_form(Hfull @ self.x + cdfull - self.z, psd_wrap(Qzfull))
        else:
            Hfull = block_diag(*[self.H for j in range(self.N + 1)])
            J += cp.quad_form(Hfull @ self.x - self.z, psd_wrap(Qzfull))

        # Add optional terminal cost
        # TODO: This needs to also be modified for nonlinear observation model
        if self.Qzf is not None:
    ...
    def set_constraints(self):
        ...
        # TODO: May need to impose trust region constraints on z as well. Although
        # TODO: since z is dependent on x, there is an implicit trust region on x already.
